...
class UnaryOp(AST):
    def __init__(self, op, child: AST):
...
class Parser(object):
    ...
    def factor(self):
        ...
        factor: (ADD | MINUS) factor | INTEGER | LPARAM expr RPARAM
        '''
        token = self.current_token
        if token.type in set([ADD, MINUS]):
            if token.type == ADD:
                self.__eat(ADD)
            elif token.type == MINUS:
                self.__eat(MINUS)
...
# Utils
def print_post_order(node, level=0, un=1):
    if isinstance(node, BinOp):
        if node.left is not None:
            print_post_order(node.left, level+1, un)
        if node.right is not None:
            print_post_order(node.right, level+1, un)
        print(f"{'-'*level} {node.token.value}")
    if isinstance(node, UnaryOp):
        print_post_order(node.child, level, un+1)
        print(f"{'-'*level}{'*'*un} {node.token.value}")
